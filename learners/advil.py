...
def f_update(obs, acts, pi, f, f_opt, prog):
    ...
    gp = gradient_penalty(learner_sa, expert_sa, f)
    f_opt.zero_grad()
...
def advil_training(data_loader, env, iters=int(1e5)):
    ...
    if data_loader.dataset.is_normalized:
        pi = AdVILPolicy(env, data_loader.dataset.mean, data_loader.dataset.std)
    else:
        pi = AdVILPolicy(env)
    f = AdVILDiscriminator(env)
    pi_opt = optim.Adam(pi.parameters(), lr=8e-6)

    last_loss = 0
    f_opt = optim.Adam(f.parameters(), lr=8e-4)
